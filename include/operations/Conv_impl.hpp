...
{
    bool useBias = false;
    if(node.input().size() == 3) useBias = true;
    ...
    vector<size_t> v;
    size_t a = dag.Add<mlpack::GroupedConvolution>(maps, kernelWidth, kernelHeight, group, strideWidth, strideHeight, padW, padH, paddingType, useBias);
    cout<<"added grouped conv"<<endl;
    // if (group == 1)
    // {
    //     size_t a = dag.Add<mlpack::Convolution>(maps, kernelWidth, kernelHeight, strideWidth, strideHeight, padW, padH, paddingType, useBias);
    //     cout << "Added mlpack::Conv Layer" << endl;
    //     v.push_back(a);
    // }
    // else
    // {
    //     size_t a = dag.Add<mlpack::Convolution>(maps, kernelWidth, kernelHeight, group, strideWidth, strideHeight, padW, padH, paddingType, useBias);
                          ...
                          std::map<std::string, double> onnxOperatorAttribute)
{
    ...
    if (node.input().size() == 2)
    {
        ...
        initializerWeights = get::Initializer(graph, initializerWeightsName);
        // layerParameters.push_back(arma::conv_to<arma::mat>::from(get::ConvertToColumnMajor(initializerWeights)));
        dag.Network()[layerIndex[0]]->Parameters() = arma::conv_to<arma::mat>::from(get::ConvertToColumnMajor(initializerWeights));
    }
    else if (node.input().size() == 3)
    {
        string initializerWeightsName = node.input(1);
