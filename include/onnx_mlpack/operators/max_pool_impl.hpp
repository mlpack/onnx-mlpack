    ...
    std::map<std::string, double> onnxOperatorAttribute)
{
  ...
  // size_t totalVerticalPadding = (strideWidth - 1) * dims[0] + kernelWidth -
  //     strideWidth;
  // size_t totalHorizontalPadding = (strideHeight - 1) * dims[1] + kernelHeight
  //     - strideHeight;
  ...
  // mlpack::Padding *padding = new mlpack::Padding(padWLeft, padWRight,
  //     padHTop, padHBottom);
  ...
  size_t b = dag.Add<mlpack::MaxPooling>(kernelWidth, kernelHeight, strideWidth,
      strideHeight, floor);
  std::cout << "Added mlpack::Padding Layer" << std::endl;
  std::cout << "Added mlpack::MaxPool Layer" << std::endl;
  dag.Connect(a, b);
  return {a, b};
